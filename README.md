...
- **即来即用**：无需登录注册，打开即用
- **双视频比对**：并排上传老师视频和我的视频
- **本人框选锁定**：练习视频可手动框选自己，确认后 AI 只分析锁定区域
- **逐帧时间轴**：拖动查看每一帧动作
- **本地路径检测**：不填 API Key 也会用浏览器 Canvas 抽帧，计算老师/用户动作路径
- **AI 复盘报告**：绿色标准路径 + 红色偏差路径，3 个关键问题 + 练习计划
- **可选真实 AI**：填入 Gemini API Key 后，会在本地路径检测基础上调用多模态模型生成总结
...
- v0.2：即来即用 H5，双视频同步 + 本地路径检测 + Mock/AI 总结（当前）
- v0.3：H5 + 后端 API + 云端历史
- v0.4：真实 AI 抽帧分析 POC
- v0.5：微信小程序测试版
